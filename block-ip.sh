...
$RAW_V6"
    ...
    if [ "$NFT_COUNT" -eq 0 ]; then
        ...
        echo "--------------------------------------------------------------"
        echo "$CLEAN_DATA" | sort -t' ' -k2 -r | head -n 5 | awk '{printf "%-45s %s\n", $1, $2}'
